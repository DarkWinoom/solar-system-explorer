 ...
 *
 * 3 件事(测试细化偏好):
 *   1. Intl 命中(28 城市表)→ 用 Intl,IP API 不调
 *   2. Intl 未知 → IP API 兜底
...
describe("locate", () => {
  ...
  it("falls back to ipapi when Intl timezone is unknown", async () => {
    _setGetTimezoneOverrideForTest(() => "Pacific/Marquesas"); // 不在 28 城市表
  ...
  it("falls back to ipwho.is when ipapi fails AND Intl unknown", async () => {
    ...
    globalThis.fetch = vi
      .fn()
      .mockResolvedValueOnce({ ok: false, status: 500 } as Response) // ipapi 失败
